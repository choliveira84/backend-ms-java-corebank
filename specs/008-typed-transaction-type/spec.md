...
**Independent Test**: Enviar requisições de autorização com `DEBIT` e `TRANSFER`, além de valores inválidos, e verificar que somente os tipos suportados seguem para autorização.
...
3. **Given** uma requisição de autorização com tipo `PIX`, **When** a API recebe o payload, **Then** a requisição é rejeitada como tipo não suportado, sem executar a autorização.
...
- **FR-001**: The transaction authorization request MUST accept only the supported transaction types `DEBIT` and `TRANSFER`.
...
- **Transaction Type**: A finite business vocabulary identifying the supported operation kind, currently `DEBIT` or `TRANSFER`.
...
- **SC-001**: 100% of authorization requests using `DEBIT` or `TRANSFER` pass type validation and retain the existing business flow.
...
- The currently supported values are `DEBIT` and `TRANSFER`, based on the existing transaction contract and requirements documentation.
